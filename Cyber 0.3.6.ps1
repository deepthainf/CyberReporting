...
# ───────────────────────────────────────────────────────────────────────────────
# Checking if an existing Graph connection is active and disconnecting if so
# ───────────────────────────────────────────────────────────────────────────────
if ((Get-MgContext).Account) { Disconnect-MgGraph  } else 

{ Write-Host "Not connected to MS Graph" -ForegroundColor Green }
...
Write-Host "Connected to '$((Get-MgOrganization).DisplayName)' successfully!" -ForegroundColor Green 

$connectedacc = (Get-MgContext).Account

Write-Host "Connecting to Exchange Online..........." -ForegroundColor Yellow 
Connect-ExchangeOnline -UserPrincipalName $connectedacc -ErrorAction Stop -ShowBanner:$false -WarningAction SilentlyContinue
...
Write-Host "Querying Intune Discovered apps..........." -ForegroundColor Yellow
...
# 3. Process ALL entries in the ZIP (in case data is split across multiple CSVs)
$allData = New-Object System.Collections.Generic.List[object]

Write-Host "Found $($zip.Entries.Count) file(s) in the ZIP archive" -ForegroundColor Cyan

foreach ($entry in $zip.Entries) {
    if ($entry.Name -like "*.csv") {
        Write-Host "  Processing: $($entry.Name) ..." -ForegroundColor Gray
        $csvText = [System.IO.StreamReader]::new($entry.Open()).ReadToEnd()
        $csvData = $csvText | ConvertFrom-Csv
        
        foreach ($row in $csvData) {
            [void]$allData.Add($row)
        }
    }
}
...
Write-Host "Total records retrieved: $($report.Count)" -ForegroundColor Green
$report| Export-Excel -Path $reportcsv -WorksheetName 'Intune Discovered Apps' `
                       -AutoSize `
                       -FreezeTopRow `
                       -BoldTopRow `
                       -AutoFilter
